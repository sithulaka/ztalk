...
class ChatWindow(ctk.CTk):
    ...
    def setup_users_list(self):
        ...
        self.settings_btn = ctk.CTkButton(self.users_controls, 
                                         text="⚙️",
                                         width=40,
                                         height=32,
                                         corner_radius=8,
    ...
    def send_message(self):
        ...
        if self.msg_type.get() == "broadcast":
            ...
            try:
                selection = self.users_list.get("sel.first", "sel.last").strip()
                username = selection.split(" ", 1)[1]  # Remove the bullet point
                self.send_private_msg(username, message)
                self.add_message(f"You → {username}", message, "#2196F3")  # Blue for own messages
            except tk.TclError:
                self.add_message("System", "Please select a user from the list for private messages", "#F44336")  # Red for errors
                return
    ...
    def ask_username(self):
        ...
        # Disable main window elements temporarily
        for widget in self.winfo_children():
            widget.destroy()
            
        # Create a simple form for username entry
        frame = ctk.CTkFrame(self, fg_color=self.colors["chat_bg"])
        frame.pack(fill="both", expand=True)
        
        # Title
        title_label = ctk.CTkLabel(frame, text="Welcome to ZTalk",
                                 font=ctk.CTkFont(size=24, weight="bold"),
                                 text_color=self.colors["text_light"])
        title_label.pack(pady=(100, 20))
        
        subtitle = ctk.CTkLabel(frame, text="Please enter your username to continue",
                              font=ctk.CTkFont(size=14),
                              text_color=self.colors["text_gray"])
        subtitle.pack(pady=(0, 40))
        
        # Username entry
        entry_frame = ctk.CTkFrame(frame, fg_color=self.colors["input_bg"], corner_radius=8)
        entry_frame.pack(padx=50, fill="x", pady=10)
        
        self.username_entry = ctk.CTkEntry(entry_frame, 
                                        placeholder_text="Username",
                                        height=40,
                                        font=ctk.CTkFont(size=14),
                                        fg_color="transparent",
                                        border_width=0)
        self.username_entry.pack(fill="x", padx=15, pady=5)
        self.username_entry.bind("<Return>", self.on_username_submit)
        
        # Submit button
        submit_btn = ctk.CTkButton(frame, 
                                 text="Continue",
                                 command=self.on_username_submit,
                                 fg_color=self.colors["accent"],
                                 hover_color=self.colors["accent_hover"],
                                 height=40,
                                 corner_radius=8,
                                 font=ctk.CTkFont(size=14, weight="bold"))
        submit_btn.pack(pady=20, padx=50, fill="x")
        
        # Focus on entry
        self.username_entry.focus()
        
    def on_username_submit(self, event=None):
        ...
        if hasattr(self, "on_username_set") and callable(self.on_username_set):
            self.on_username_set(username)
        
    def initialize_ui(self):
        """Initialize the main UI after username is set"""
        ...
        self.sidebar.grid_rowconfigure(8, weight=1)  # Allow last row to expand
        ...
        # Format the chat display for modern message styles
        self.format_chat_display()

        # Message input area
        self.setup_input_area()
        
        # Create utility buttons area (SSH, etc.)
        self.setup_utility_buttons()
        
        # Add a network status indicator
        self.setup_network_status()

        # Initial users refresh
        self.refresh_users()
        
        # Auto-refresh timer for users list (every 5 seconds)
        self._auto_refresh_id = self.after(5000, self.auto_refresh_users)
        
        # Welcome message
        self.after(500, lambda: self.add_system_message(f"Welcome to ZTalk, {self.username}!")) 