...
# Allowlist for registry key paths that ZTalk is permitted to write to
_REGISTRY_WRITE_ALLOWLIST = [
    r"HKEY_CURRENT_USER\Software\ZTalk",
]


def _validate_registry_path(root_key, subkey: str) -> bool:
    """Validate that a registry path is within the allowed set.

    Only allows writes under HKEY_CURRENT_USER\\Software\\ZTalk.

    Args:
        root_key: The winreg root key constant (e.g. winreg.HKEY_CURRENT_USER).
    ...
    root_name = root_map.get(root_key, "UNKNOWN")
    full_path = f"{root_name}\\{subkey}".rstrip("\\")

    for allowed in _REGISTRY_WRITE_ALLOWLIST:
        if full_path.upper().startswith(allowed.upper()):
            return True

    logger.error(f"Registry write blocked: {full_path} is not in the allowlist")
    return False


def _escape_nsis_path(path: str) -> str:
    """Escape a file path for safe interpolation into an NSIS script.

    Doubles backslashes and escapes special NSIS characters ($, ", `).

    Args:
        path: The raw file path.

    Returns:
        The escaped string safe for NSIS script embedding.
    """
    # Double backslashes first (before adding new ones from other escapes)
    path = path.replace("\\", "\\\\")
    # Escape NSIS special characters
    path = path.replace("$", "$$")
    path = path.replace('"', '$\\"')
    path = path.replace("`", "$\\`")
    return path
...
def build_windows_installer(source_dir: str, output_dir: str, 
    ...
    try:
        ...
        # Escape all interpolated paths for NSIS safety
        esc_installer_path = _escape_nsis_path(installer_path)
        esc_icon_path = _escape_nsis_path(icon_path or os.path.join(source_dir, 'icon.ico'))
        esc_license_path = _escape_nsis_path(os.path.join(source_dir, 'LICENSE'))
        esc_source_dir = _escape_nsis_path(source_dir)
        esc_app_name = _escape_nsis_path(app_name)
        esc_app_name_safe = _escape_nsis_path(app_name_safe)
        esc_version = _escape_nsis_path(version)

        # Generate NSIS script
        with open(script_path, 'w') as f:
            f.write(f"""
; {esc_app_name} Installer Script
; Generated by ZTalk
...
Name "{esc_app_name}"
OutFile "{esc_installer_path}"
InstallDir "$PROGRAMFILES\\{esc_app_name}"
InstallDirRegKey HKLM "Software\\{esc_app_name_safe}" "Install_Dir"
RequestExecutionLevel admin

; UI settings
!define MUI_ABORTWARNING
!define MUI_ICON "{esc_icon_path}"

; Pages
!insertmacro MUI_PAGE_WELCOME
!insertmacro MUI_PAGE_LICENSE "{esc_license_path}"
...
Section "MainSection" SEC01
    ...
    File /r "{esc_source_dir}\\*.*"

    ; Create uninstaller
    WriteUninstaller "$INSTDIR\\uninstall.exe"

    ; Create start menu shortcut
    CreateDirectory "$SMPROGRAMS\\{esc_app_name}"
    CreateShortCut "$SMPROGRAMS\\{esc_app_name}\\{esc_app_name}.lnk" "$INSTDIR\\{esc_app_name_safe}.exe"
    CreateShortCut "$SMPROGRAMS\\{esc_app_name}\\Uninstall.lnk" "$INSTDIR\\uninstall.exe"

    ; Create desktop shortcut
    CreateShortCut "$DESKTOP\\{esc_app_name}.lnk" "$INSTDIR\\{esc_app_name_safe}.exe"

    ; Write registry keys for uninstall info
    WriteRegStr HKLM "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\{esc_app_name_safe}" \\
                 "DisplayName" "{esc_app_name}"
    WriteRegStr HKLM "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\{esc_app_name_safe}" \\
                 "UninstallString" "$INSTDIR\\uninstall.exe"
    WriteRegStr HKLM "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\{esc_app_name_safe}" \\
                 "DisplayVersion" "{esc_version}"
    WriteRegStr HKLM "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\{esc_app_name_safe}" \\
                 "Publisher" "ZTalk Team"

    ; Register firewall rule
    ExecWait 'netsh advfirewall firewall add rule name="{esc_app_name}" dir=in action=allow program="$INSTDIR\\{esc_app_name_safe}.exe" enable=yes profile=any'
SectionEnd

Section "Uninstall"
    ; Remove firewall rule
    ExecWait 'netsh advfirewall firewall delete rule name="{esc_app_name}" program="$INSTDIR\\{esc_app_name_safe}.exe"'

    ; Remove shortcuts
    Delete "$DESKTOP\\{esc_app_name}.lnk"
    Delete "$SMPROGRAMS\\{esc_app_name}\\*.*"
    RMDir "$SMPROGRAMS\\{esc_app_name}"

    ; Remove files
    RMDir /r "$INSTDIR"

    ; Remove registry keys
    DeleteRegKey HKLM "Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\{esc_app_name_safe}"
    DeleteRegKey HKLM "Software\\{esc_app_name_safe}"
SectionEnd
            """)
...
def set_file_association(extension: str, app_path: str, description: str) -> bool:
    ...
    try:
        ...
        # Validate all registry paths against allowlist
        paths_to_write = [
            ...
            (winreg.HKEY_CLASSES_ROOT, f".{extension}"),
        ]
        for root_key, subkey in paths_to_write:
            if not _validate_registry_path(root_key, subkey):
                logger.error(f"Registry write to {subkey} blocked by allowlist")
                return False
